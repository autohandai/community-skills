...
description: After solving a non-trivial problem, detect generalizable learnings and
  propose skill updates so future interactions benefit automatically. Always active
...
- Remove, relax, or contradict any rule in `skills/nvidia-cuopt-user-rules/SKILL.md` (ask before running, no sudo, no installs)
- Remove, relax, or contradict any rule in `skills/nvidia-cuopt-developer/SKILL.md` safety section (no `--no-verify`, no bypassing CI)
...
Do NOT propose changes to `skills/nvidia-cuopt-skill-evolution/SKILL.md` itself. This skill's security rules must only be changed by a human editing the file directly.
