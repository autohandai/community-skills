---
name: nvidia-nemo-gym-pivot-datasets
description: Use when creating, validating, or documenting Nemo Gym pivot datasets
  from rollout, trajectory, chat-completion, Responses API, or tool-call artifacts.
  Covers Gym Responses-style row conversion, pivot selection, single-step tool-use
  configs, agent_ref alignment, verifier knobs, expected-action row contracts, and
  train/eval usage.
