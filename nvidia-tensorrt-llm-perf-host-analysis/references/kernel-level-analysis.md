...
When a single non-capturable op triggers `stop_partition = True`, it can kill graph capture for **all subsequent ops** in the same scope. See the GRAPH_EXPAND pattern in the optimization skill's [patterns/gpu-graph.md](../../nvidia-tensorrt-llm-perf-host-optimization/references/patterns/gpu-graph.md) for fix strategies.
