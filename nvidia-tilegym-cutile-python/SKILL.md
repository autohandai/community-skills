...
description: Expert cuTile programming assistant. Write high-performance GPU kernels
  using cuTile's tile-based programming model with proper validation and optimization.
  Supports deep agent orchestration for complex multi-kernel tasks.
...
- **Inside a TileGym checkout** (`<repo>/.agents/skills/nvidia-tilegym-cutile-python/`, or `<repo>/.claude/skills/nvidia-tilegym-cutile-python/` via the backward-compat symlink) — TileGym ops are at `<repo>/src/tilegym/ops/cutile/`.
- **Installed elsewhere** (e.g. `~/.agents/skills/nvidia-tilegym-cutile-python/`, `~/.claude/skills/nvidia-tilegym-cutile-python/`, or inside a different repo) — clone TileGym once to `${TILEGYM_SKILL_CACHE_DIR:-~/.cache/tilegym}/TileGym` and use its `src/tilegym/ops/cutile/`.
