...
Path looks like `<repo>/.agents/skills/nvidia-tilegym-cutile-python/` (or `<repo>/.claude/skills/nvidia-tilegym-cutile-python/` via the backward-compat symlink). The enclosing repo **is** TileGym. No clone needed — use it directly:
...
Path looks like `~/.agents/skills/nvidia-tilegym-cutile-python/` or `~/.claude/skills/nvidia-tilegym-cutile-python/`, or the skill is inside some other repo that does not ship `src/tilegym/`. TileGym is not adjacent; clone it once on first use to the cache directory and use it from there:
