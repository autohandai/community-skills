...
description: Bump a pinned dependency (TransformerEngine, Megatron-LM, NRX, etc.),
  regenerate the lockfile, open a PR, and drive it to green by attaching a watchdog
  to the "CICD NeMo" workflow and quarantining failing functional tests as flaky until
  the run is green.
license: Apache-2.0 AND CC-BY-4.0
metadata:
  when_to_use: Bumping a dependency pin in `pyproject.toml` or `uv.lock` and shepherding
    the PR to green. 'bump TE', 'bump transformer-engine', 'update TE pin', 'bump
    submodule', 'update lock file', 'bump dependency PR', 'watch CI for a bump', 'quarantine
    flaky tests after bump', 'run all tests for this bump'.
  author: NVIDIA
---
...
- @skills/nvidia-megatron-bridge-build-and-dependency/SKILL.md — `uv lock` mechanics, container choice
- @skills/nvidia-megatron-bridge-cicd/SKILL.md — how `copy-pr-bot` and `/ok to test` work
- @skills/nvidia-megatron-bridge-testing/SKILL.md — `active/` vs `flaky/` directory layout, `git mv` quarantine recipe
...
@skills/nvidia-megatron-bridge-build-and-dependency/SKILL.md "Regenerating uv.lock". Then
...
@skills/nvidia-megatron-bridge-cicd/SKILL.md "Commit and PR Workflow". For a bump:
...
Trigger mechanics live in @skills/nvidia-megatron-bridge-cicd/SKILL.md "How CI Is Triggered".
...
2. **Move the launch script to `flaky/`** per @skills/nvidia-megatron-bridge-testing/SKILL.md
