...
If remote but no config, ask user for: hostname, SSH username, SSH key path, remote workdir. Create `~/.config/modelopt/clusters.yaml` (see `common/remote-execution.md` for format).
...
If `MODELOPT_WORKSPACE_ROOT` is set, read `common/workspace-management.md` before proceeding.
